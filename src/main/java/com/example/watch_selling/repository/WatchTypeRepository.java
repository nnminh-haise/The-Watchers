...
public interface WatchTypeRepository extends JpaRepository<WatchType, UUID> {
    ...
    @Query(
        nativeQuery = true,
        value = "SELECT * FROM watch_type WHERE id = :id AND is_deleted = false"
    )
    ...
    @Query(
        nativeQuery = true,
        value = "SELECT * FROM watch_type WHERE name = :name AND is_deleted = false"
    )
    ...
    @Query(
        nativeQuery = true,
        value = "SELECT * FROM watch_type WHERE is_deleted = false"
    )
    public List<WatchType> findAll();

    @Query(
        nativeQuery = true,
        value = "UPDATE watch_type SET name = :name WHERE id = :id AND is_deleted = false"
    )
    @Modifying
    @Transactional
