...
public interface WatchBrandRepository extends JpaRepository<WatchBrand, UUID> {
    ...
    @Query(
        nativeQuery = true,
        value = "SELECT * FROM watch_brand WHERE id = :id AND is_deleted = false"
    )
    ...
    @Query(
        nativeQuery = true,
        value = "SELECT * FROM watch_brand WHERE name = :name AND is_deleted = false"
    )
    ...
    @Query(
        nativeQuery = true,
        value = "SELECT * FROM watch_brand WHERE is_deleted = false"
    )
    public List<WatchBrand> findAll();

    @Query(
        nativeQuery = true,
        value = "UPDATE watch_brand SET name = :name WHERE id = :id AND is_deleted = false"
    )
    @Modifying
    @Transactional
